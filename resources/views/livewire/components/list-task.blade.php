<div>
    ...
    <div class="row">
        <div class="col-lg-12 mb-3 col-md-12">
            <div class="card shadow-none bg-transparent border border-{{ $color }} mb-3">
                <div class="card-body">
                    <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ $title }}</h5>
                        <div class="card-title-elements ms-auto">
                            <span class="badge bg-{{ $color }} rounded-pill">{{ $listTaskCount }}</span>
                        </div>
                    </div>
                    <div class="accordion mt-3">
                        @forelse ($listTask as $data)
                            <div class="accordion-item card">
                                <li class="d-flex align-items-center accordion-button collapsed"
                                    data-bs-toggle="collapse"
                                    data-bs-target="#listTask-{{ $prefix }}-{{ $data->id }}"
                                    aria-expanded="false" role="button">
                                    <div class="badge bg-label-{{ $color }} me-3 rounded p-2">
                                    ...
                                    <div
                                        ...
                                        <div class="me-2">
                                            ...
                                            <span
                                                class="badge rounded-pill bg-label-{{ $color }}">{{ $data->status }}</span>
                                            <span
                                                class="badge rounded-pill bg-label-{{ $color }}">{{ $data->pekerjaan }}</span>
                                        </div>
                                    </div>
                                </li>
                                <div id="listTask-{{ $prefix }}-{{ $data->id }}"
                                    class="accordion-collapse collapse">
                                    <ul class="p-2 m-1">
                                        <li class="d-flex justify-content-center align-items-center mb-3">
                                            <a href="{{ route($this->targetRoute, ['state' => $this->prefix, 'id' => $data->instruction_id]) }}"
                                                class="btn btn-outline-{{ $color }} me-3">
                                                <span class="ti-xs ti ti-clock-play me-1"></span>Mulai Kerjakan
                                            </a>
                                            <button data-bs-toggle="offcanvas" data-bs-target="#offcanvasScroll"
                                                aria-controls="offcanvasScroll" @click="$dispatch('show-off-canvas', {id:{{ $data->instruction_id }}})"
                                        ...
                                        <li class="d-flex align-items-center mb-3 mt-3">
                                            ...
                                            <div
                                                class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                                                <div class="me-2">
                                                    <h6 class="mb-0">{{ $data->instruction->panjang_barang }} cm x
                                                        {{ $data->instruction->lebar_barang }} cm</h6>
